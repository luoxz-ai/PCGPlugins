...
/**
 * 调墙高的抓手（计划 D5 的第二个自由度，用户裁决 2026-09-06）：**一个横放的"窗框"**，
 * 沿上下拖它就改 `WallHeight`。
 *
 * 观感由用户点名：**四根细长长方体首尾相接围成一个矩形框**，长宽 = footprint × `FrameScale`
 * （默认 1.2 倍）。框停在檐口高度上，所以它同时是"墙有多高"的读数 —— 往上拖房子长高、
 * 往下拖变矮，框始终贴着墙顶。
 *
 * ## 与拉尺寸抓手（`ACSHouseResizeHandleActor`）的分工
 *
 * 那四个锥子管**水平**（各推一面墙，改 footprint）；这一个框管**竖直**（改墙高）。
 * 两者同属 `ACSHouseHandleActor` 一族，生命周期、宿主纪律、记账量法完全一致，
 * 只是投影轴不同：锥子投到自己那面墙的外法线上，框投到**世界 Z** 上。
 *
 * ## 为什么是一个 actor 带四根条子，而不是四个 actor
 *
 * 它只有**一个自由度**。拆成四个 actor 的话，用户抓哪根都在改同一个量，四个 gizmo 互相打架；
 * 而且框的四条边必须永远闭合，分给四个 actor 就得再写一套同步。拉尺寸那边正相反 ——
 * 四面墙是四个独立自由度，所以那边才一面墙一个 actor。
 *
 * ## 没有父子回路
 *
 * `PushHeight` 只改 `WallHeight`，**不动 actor 变换**，所以不存在拉尺寸那边"父级移动 Applied/2
 * 把抓手一起带走"的 2× 回路。记账量法照旧保留：它同时还管着"顶在 `MinWallHeight` 上时
 * 不许攒残差"这一条（与 `CSHouseResize.h` 的返回值契约同一个理由）。
 */
...
public:
	...
	/**
	 * 认宿主 + 摆到规范位置。`EnterResizeMode` 生成之后立刻调，只调一次。
	 *
	 * ⚠️ attach 必须在这之前完成（`SnapToCanonical` 写的是世界位置）。
	 */
	void InitializeHandle(ACSHouseActor* InHost);

	/**
	 * 规范位置（世界）：房心正上方、檐口高度（= `WallHeight`）处。
	...
	/**
	 * **交互的唯一执行面**：把竖直位移翻成一次改墙高。返回实际生效的高度变化 cm。
	 *
	 * 只取**世界 Z** 分量，水平分量忽略（用户把框拖歪不该改高度；歪掉的部分在回位时清掉）。
	 * `bFinished` 直通 `PushHeight`。
	 */
...
private:
	...
	/**
	 * 记账量：上一次已经被 `PushHeight` 消费掉的世界位置。
	...
	/** 框的四条边。下标即边号，与 `CSHouse_GetEdge` / `CSHouseResize_EdgeOuterLocal` 同号。 */
	UPROPERTY(Transient)
